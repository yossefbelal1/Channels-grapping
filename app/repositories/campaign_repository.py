        ...
        with get_db_cursor() as cur:
            cur.execute("""
                SELECT c.id, c.created_at, c.message_text, c.media_path, c.status,
                       ...
                       COUNT(CASE WHEN cl.status IN ('pending', 'pending_review') THEN 1 END) as pending_review_count,
                       COUNT(CASE WHEN cl.status = 'approved' THEN 1 END) as approved_count,
                       COUNT(CASE WHEN cl.status IN ('pending', 'pending_review', 'approved') THEN 1 END) as pending_count,
                       COUNT(CASE WHEN cl.status IN ('pending', 'pending_review', 'approved') AND COALESCE(l.outreach_priority, cl.priority, 'P3') = 'P0' THEN 1 END) as p0_pending_count,
                       COUNT(CASE WHEN cl.status IN ('pending', 'pending_review', 'approved') AND COALESCE(l.outreach_priority, cl.priority, 'P3') = 'P1' THEN 1 END) as p1_pending_count,
                       COUNT(CASE WHEN cl.status IN ('pending', 'pending_review', 'approved') AND COALESCE(l.outreach_priority, cl.priority, 'P3') = 'P2' THEN 1 END) as p2_pending_count,
                       COUNT(CASE WHEN cl.status IN ('pending', 'pending_review', 'approved') AND COALESCE(l.outreach_priority, cl.priority, 'P3') = 'P3' THEN 1 END) as p3_pending_count,
                       COUNT(CASE WHEN cl.status IN ('pending', 'pending_review', 'approved') AND COALESCE(l.outreach_priority, cl.priority, 'P3') = 'P4' THEN 1 END) as p4_pending_count,
